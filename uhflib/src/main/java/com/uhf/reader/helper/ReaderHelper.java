...
import com.uhf.reader.base.HEAD;
...
public class ReaderHelper {
     ...
     */
	public static ReaderHelper getDefaultHelper() throws Exception {//在获取读写帮助类的时候建立了和读写器主键的连接（JNI操作）
		if (mReaderHelper == null){//TODO 为空！
			Log.e("gyReader", "getDefaultHelper: mReaderHelper == null");
		} else if (mContext ==null){
			Log.e("gyReader", "getDefaultHelper: mContext == null");
		}
	...
	private void refreshReaderSetting(byte btCmd, ReaderSetting curReaderSetting) {
		...
		mLocalBroadcastManager.sendBroadcast(itent);
		Log.e("gy.log", "refreshReaderSetting: 发送广播BROADCAST_REFRESH_READER_SETTING");
	};
		...
		if (mReader == null) {
			...
			mReader = new ReaderBase(in, out) {
				...
				public void analyData(MessageTran msgTran) {
					...
					Log.e("gy.log", "analyData: "+ msgTran.getAryData());
					Log.e("gy.log", "analyData: getCmd = "+ msgTran.getCmd());

                    Intent itent = new Intent("gy_guangbo");
                    itent.putExtra("gy", "发送的广播");
                    mLocalBroadcastManager.sendBroadcast(itent);
				}
	...
	private void analyData(MessageTran msgTran) {
		...
		Log.e("gy.log", "msgTran.getCmd() = "+ msgTran.getCmd());//SET_WORK_ANTENNA = 0x74~116
        ...
        case CMD.READ_TAG:
			...
			Log.e("gy.log", "【CMD.READ_TAG=(byte) 0x81】analyData: "+ msgTran.getCmd());
        	break;
        case CMD.WRITE_TAG:
        ...
        case CMD.GET_ACCESS_EPC_MATCH:
        	...
        	break;
        case CMD.REAL_TIME_INVENTORY://TODO 实时存盘
